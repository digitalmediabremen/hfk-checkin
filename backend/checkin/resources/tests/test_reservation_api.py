...
from checkin.resources.models import (Reservation, Resource, ResourceGroup, Attachment)
...
DEFAULT_RESERVATION_EXTRA_FIELDS = ('reserver_name', 'reserver_phone_number', 'reserver_address_street',
                                    ...
                                    'reserver_pk', 'number_of_participants', 'reserver_email_address')
...
DEFAULT_REQUIRED_RESERVATION_EXTRA_FIELDS = ('reserver_name', 'reserver_phone_number', 'reserver_address_street',
                                             ...
                                             'reserver_pk', 'reserver_email_address')
...
#         start='2115-04-01',
#         end='2115-05-01',
#         resource_pk=resource_in_unit.pk,
#         name='test_period'
#     )
...
def reservation_data(resource_in_unit):
    return {
        'resource_uuid': resource_in_unit.pk,
...
def test_authenticated_user_can_make_reservation(api_client, list_url, reservation_data, resource_in_unit, user):
    ...
    assert len(reservation.identifier) == 7
    assert reservation.number_of_attendees == 1
    assert reservation.number_of_extra_attendees == 0
    assert reservation.resource == resource_in_unit
    assert reservation.agreed_to_phone_contact == False
    assert reservation.exclusive_resource_usage == False
...
@pytest.mark.django_db
def test_authenticated_user_can_make_reservation_exclusive_usage(api_client, list_url, reservation_data, resource_in_unit, user):
    """
    Tests that an authenticated user can create a reservation.
    """
    api_client.force_authenticate(user=user)
    reservation_data['exclusive_resource_usage'] = True

    response = api_client.post(list_url, data=reservation_data)
    assert response.status_code == 201, "Request failed with: %s" % (str(response.content, 'utf8'))
    reservation = Reservation.objects.filter(user=user).latest('created_at')
    assert reservation.exclusive_resource_usage == True


@pytest.mark.django_db
def test_authenticated_user_can_make_reservation_with_phone_contact(api_client, list_url, reservation_data, resource_in_unit, user):
    """
    Tests that an authenticated user can create a reservation.
    """
    api_client.force_authenticate(user=user)
    reservation_data['agreed_to_phone_contact'] = True

    response = api_client.post(list_url, data=reservation_data)
    assert response.status_code == 201, "Request failed with: %s" % (str(response.content, 'utf8'))
    reservation = Reservation.objects.filter(user=user).latest('created_at')
    assert reservation.agreed_to_phone_contact == True
...
@pytest.mark.django_db
def test_authenticated_user_can_retrieve_reservation(api_client, detail_url, reservation, user):
    """
    Tests that an authenticated user can modify her own reservation
    """
    api_client.force_authenticate(user=user)
    ...
    assert response.status_code == 200
    reservation = Reservation.objects.get(pk=response.data['uuid'])
    assert str(reservation.uuid) == response.data['uuid']

@pytest.mark.django_db
def test_authenticated_user_can_cancel_reservation(api_client, detail_url, reservation, user):
    """
    Tests that an authenticated user can modify her own reservation
    """
    api_client.force_authenticate(user=user)
...
#     assert Reservation.objects.filter(pk=reservation_pk).count() == 1
...
#     api_client.force_authenticate(user=user)
#     reservation_data['message'] = 'test comment'
#     response = api_client.post(list_url, data=reservation_data)
#     assert response.status_code == 400
#
...
#     assert response.data['message'] == 'test comment'
#
#     api_client.force_authenticate(user=user)
#     response = api_client.get(response.data['url'])
...
#     assert response.data['count'] == 1
#     assert response.data['results'][0]['id'] == reservation2.pk
#
#
# @pytest.mark.django_db
...
#     assert response.data['count'] == 2
#     assert {reservation.pk, reservation2.pk}.issubset(set(res['id'] for res in response.data['results']))
#
...
#     assert response.status_code == 200
#     reservation_ids = set([res['id'] for res in response.data['results']])
#     assert reservation_ids == set(reservations_in_all_states[state].pk for state in expected_states)
#
#
...
#         'Normal reservation created subject.',
#         user.email,
#         'image url: https://foo.bar/baz/resource_image/{}'.format(main_image.pk),
#         html_body='image: <img src="https://foo.bar/baz/resource_image/{}">'.format(last_ground_plan_image.pk),
#     )
#
#
# @pytest.mark.parametrize('perm_type', ['unit', 'resource_group'])
...
#     assert_response_objects(response, (reservation, reservation2))
#
#     response = user_api_client.get(list_url + '?unit=' + resource_in_unit2.unit.pk)
#     assert response.status_code == 200
#     assert_response_objects(response, reservation2)
#
...
#
#     response = api_client.get(list_url + '?resource={}'.format(resource_in_unit.pk))
#     assert response.status_code == 200
#     assert_response_objects(response, reservation)
#
#     response = api_client.get(list_url + '?resource={},{}'.format(resource_in_unit.pk, resource_in_unit2.pk))
#     assert response.status_code == 200
#     assert_response_objects(response, (reservation, reservation2))
#
#
...
#     reservation.message = 'To be a foo or not to be a foo, that is the question'
#     reservation.save()
#     detail_url = reverse('reservation-detail', kwargs={'pk': reservation.pk})
#     api_client.force_authenticate(user=unit_manager_user)
#
#     response = api_client.get(detail_url)
#     assert response.status_code == 200
#     assert response.data['message'] == 'To be a foo or not to be a foo, that is the question'
#
#
# @pytest.mark.django_db
