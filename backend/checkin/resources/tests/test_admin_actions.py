...
#from checkin.users.admin import anonymize_user_data
...
#     user_pk = user.pk
#
#     SocialAccount.objects.create(user=user, upk=original_uuid, provider='helsinki')
...
#     assert not SocialAccount.objects.filter(user=user, upk=original_uuid).exists()
